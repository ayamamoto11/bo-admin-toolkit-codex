...
public class ReportArchiveModule {
    ...
    public List<ReportArchiveRecord> validateWorkbook(InputStream inputStream, String archiveFolderName)
        ...
        try (Workbook workbook = new XSSFWorkbook(inputStream)) {
            ...
            for (int rowIndex = sheet.getFirstRowNum() + 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                records.add(validateReport(infoStore, rowIndex + 1, cuid, reportName, folderPath, archiveFolderName, pendingNamesByTarget));
            }
        }
        return records;
    }
    ...
    public List<ReportArchiveRecord> validateKeyword(String keyword, String archiveFolderName) throws SDKException {
        ...
        String query = "SELECT TOP 100000 * FROM CI_INFOOBJECTS WHERE SI_KIND = 'Webi' AND SI_INSTANCE = 0 "
                + "AND SI_KEYWORD LIKE '%" + escapeCmsQueryValue(keyword.trim()) + "%' ORDER BY SI_NAME";
        IInfoObjects reports = infoStore.query(query);
        Map<String, Integer> pendingNamesByTarget = new HashMap<>();
        for (int index = 0; index < reports.size(); index++) {
            IInfoObject report = (IInfoObject) reports.get(index);
            records.add(toReadyRecord(infoStore, index + 1, report, report.getTitle(), "", archiveFolderName, pendingNamesByTarget));
        }
        if (records.isEmpty()) {
    ...
    public List<ReportArchiveRecord> archiveReports(List<ReportArchiveRecord> validatedRecords) throws SDKException {
        ...
        for (ReportArchiveRecord record : validatedRecords) {
            ...
            int targetFolderId = ensureFolderHierarchy(infoStore, archiveRootId,
                    removeArchiveRoot(record.getTargetFolderPath(), record.getArchiveFolderName()), folderCache);
            String finalName = uniqueReportName(infoStore, targetFolderId, report.getTitle(), report.getCUID(), renamedNamesByTarget);
            ...
            results.add(new ReportArchiveRecord(record.getRowNumber(), record.getCuid(), report.getTitle(),
                    record.getCurrentFolderPath(), record.getArchiveFolderName(), record.getTargetFolderPath(),
                    finalName, "Archived", "Report moved to archive folder."));
        }
        return results;
    }

    private ReportArchiveRecord validateReport(IInfoStore infoStore, int rowNumber, String cuid, String reportName,
            String folderPath, String archiveFolderName, Map<String, Integer> pendingNamesByTarget) throws SDKException {
        if (isBlank(cuid)) {
        ...
        if (report == null) {
            return errorRecord(rowNumber, cuid, reportName, folderPath, archiveFolderName,
                    "Report was not found or is not a WebI report.");
        }
        return toReadyRecord(infoStore, rowNumber, report, reportName, folderPath, archiveFolderName, pendingNamesByTarget);
    }

    private ReportArchiveRecord toReadyRecord(IInfoStore infoStore, int rowNumber, IInfoObject report,
            String reportName, String folderPath, String archiveFolderName, Map<String, Integer> pendingNamesByTarget)
            throws SDKException {
        ...
        return new ReportArchiveRecord(rowNumber, report.getCUID(), displayReportName, currentFolderPath,
                archiveFolderName.trim(), targetFolderPath, finalName, STATUS_READY, "Valid WebI report.");
    }

    private IInfoObject queryReportByCuid(IInfoStore infoStore, String cuid) throws SDKException {
        IInfoObjects reports = infoStore.query("SELECT TOP 1 * FROM CI_INFOOBJECTS WHERE SI_KIND = 'Webi' AND SI_CUID = '"
                + escapeCmsQueryValue(cuid) + "'");
        return reports.size() == 0 ? null : (IInfoObject) reports.get(0);
    }

    private int ensureFolderHierarchy(IInfoStore infoStore, int archiveRootId, String relativePath,
    ...
    private String uniqueReportName(IInfoStore infoStore, int parentId, String baseName, String currentCuid,
    ...
    private boolean reportNameExists(IInfoStore infoStore, int parentId, String reportName, String currentCuid)
        ...
        IInfoObjects reports = infoStore.query("SELECT TOP 1 SI_ID, SI_CUID, SI_NAME FROM CI_INFOOBJECTS "
                + "WHERE SI_KIND = 'Webi' AND SI_PARENTID = " + parentId + " AND SI_NAME = '"
                + escapeCmsQueryValue(reportName) + "'");
        if (reports.size() == 0) {
            return false;
        }
        return !currentCuid.equals(((IInfoObject) reports.get(0)).getCUID());
    }

    private String pendingUniqueReportName(String targetFolderPath, String baseName, Map<String, Integer> pendingNamesByTarget) {
    ...
    private int resolvePublicRootFolderId(IInfoStore infoStore) throws SDKException {
        ...
        IInfoObjects anyFolders = infoStore.query("SELECT TOP 1 SI_ID, SI_NAME, SI_PARENTID FROM CI_INFOOBJECTS WHERE SI_KIND = 'Folder' ORDER BY SI_ID");
        if (anyFolders.size() == 0) {
            throw new IllegalStateException("Unable to find a public root folder.");
        }
        return ((IInfoObject) anyFolders.get(0)).getID();
    }
    ...
    private String resolveFolderPath(IInfoStore infoStore, int folderId, Map<Integer, String> pathCache) throws SDKException {
        if (folderId <= 0) {
            return "";
        }
        ...
        IInfoObjects folders = infoStore.query("SELECT TOP 1 SI_ID, SI_NAME, SI_PARENTID FROM CI_INFOOBJECTS, CI_APPOBJECTS, CI_SYSTEMOBJECTS WHERE SI_ID = " + folderId);
        if (folders.size() == 0) {
            pathCache.put(folderId, "");
            return "";
        }
        IInfoObject folder = (IInfoObject) folders.get(0);
        String parentPath = resolveFolderPath(infoStore, folder.getParentID(), pathCache);
    ...
    private String readOptionalCell(Row row, Integer columnIndex, DataFormatter formatter) {
        return columnIndex == null ? "" : readCell(row, columnIndex, formatter);
    }
    ...
    private ReportArchiveRecord errorRecord(int rowNumber, String cuid, String reportName, String folderPath,
            String archiveFolderName, String message) {
        return new ReportArchiveRecord(rowNumber, cuid, reportName, folderPath, archiveFolderName, "", "", STATUS_ERROR, message);
    }

    private ReportArchiveRecord copyWithStatus(ReportArchiveRecord record, String status, String message) {
        return new ReportArchiveRecord(record.getRowNumber(), record.getCuid(), record.getReportName(),
                record.getCurrentFolderPath(), record.getArchiveFolderName(), record.getTargetFolderPath(),
                record.getFinalReportName(), status, message);
    }

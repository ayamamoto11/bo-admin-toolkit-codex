    ...
    public void export(List<ReportInventoryRecord> records, OutputStream outputStream) throws IOException {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (ReportInventoryRecord record : records) {
                ...
                row.createCell(7).setCellValue(record.getDataProviderName());
                row.createCell(8).setCellValue(record.getUniverseId());
                row.createCell(9).setCellValue(record.getUniverseName());
                row.createCell(10).setCellValue(record.getUniverseType());
                row.createCell(11).setCellValue(record.getUniverseCuid());
                row.createCell(12).setCellValue(record.getUniversePath());
            }
